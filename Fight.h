...
public:
	Fight(SDL_Renderer* ren) {
		std::vector<Button> q;
		text =  Text(36, L"К вам приближается корабль с чёрным флагом. До вас доносится крик с требованием откупа.", ren, { 100,576,500,100 });
		pikcha =  Sprite("textures/cherepok.png", ren, 300, 0, 1024, 576);
		SDL_Rect rect = { 100,676,500,100 };
		otkup = genious_random(100, 1000);
		slozhnost = genious_random(1, 10000);
		std::wstring s = L"•откупиться: " + std::to_wstring(otkup) + L"g";
		q.push_back( Button(ren, s.c_str(), 48, rect, "textures/active_fon.png"));
		rect.x += 1000;
		q.push_back( Button(ren, L"•сражаться!", 48, rect, "textures/active_fon.png"));
		//rect.y += 100;
		addButtonRow(q);
	}
	void update() {
